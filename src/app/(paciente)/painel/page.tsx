...
function DetalheAvaliacao({
  ...
  const metricas = [
    ...
    {
      label: "Soma Todas as Dobras",
      valor: n(r?.somaTodasDobras),
      unidade: "mm",
    },
    ...
    <div className="fixed inset-0 z-50 flex flex-col justify-end md:items-center md:justify-center">
      ...
      <div className="relative w-full md:max-w-lg md:mx-auto md:mb-0 bg-white md:rounded-[28px] rounded-t-[28px] shadow-2xl flex flex-col max-h-[90vh] md:max-h-[85vh]">
        ...
        <div className="flex items-start justify-between px-5 py-4 border-b border-slate-100">
          <div>
            <p className="font-mono-ui text-[10px] uppercase tracking-[0.22em] text-slate-400">
              Avaliação
            </p>
            <h2 className="text-lg font-semibold text-slate-900 mt-0.5">
              {new Date(aval.dataAvaliacao).toLocaleDateString("pt-BR", {
                day: "2-digit",
                month: "long",
                year: "numeric",
                timeZone: "UTC",
        ...
        <div className="overflow-y-auto flex-1 px-5 py-5 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {metricas.map((m) => (
              <div key={m.label} className="glass-panel rounded-[18px] p-3.5">
                <p className="font-mono-ui text-[9px] uppercase tracking-[0.18em] text-slate-400">
                  {m.label}
                </p>
                {m.valor != null && (
                ...
                {m.badge && (
                  <span
                    className={`inline-block mt-1.5 px-2 py-0.5 rounded-lg text-xs font-bold font-mono-ui uppercase tracking-[0.12em] ${corDobras(m.badge)}`}
                  >
                    {m.badge}
                  </span>
                )}
          ...
          {temSomato && (
            <div>
              <p className="font-mono-ui text-[10px] uppercase tracking-[0.22em] text-slate-400 mb-2.5">
                Somatotipo — Heath-Carter
              </p>
              <div className="grid grid-cols-3 gap-2">
                <div className="metric-card rounded-[18px] bg-gradient-to-br from-[#c96d42] to-[#d88c5b] p-4 text-white text-center">
                  <p className="font-mono-ui text-[9px] uppercase tracking-[0.18em] text-white/70">
                    Endomorfia
                  </p>
...
export default function PainelPaciente() {
  ...
  const serieRes = (
    fn: (r: NonNullable<Avaliacao["resultado"]>) => number | null | undefined,
  ) =>
    avals.map((a) => ({
      data: fmt(a.dataAvaliacao),
      valor: a.resultado ? (fn(a.resultado) ?? null) : null,
    }));
  ...
  const percG =
    (r?.formulaReferencia === "faulkner"
      ? r?.percGorduraFaulkner
      : r?.percGorduraPetroski) ?? null;
  const diffPeso = diff((a) => a.peso);
  const diffImc = diff((a) => a.resultado?.imc);
  const diffPercG = diff((a) =>
    a.resultado?.formulaReferencia === "faulkner"
      ? a.resultado?.percGorduraFaulkner
        ...
        <div className="glass-panel-strong flex flex-col min-h-screen md:min-h-[calc(100vh-3rem)] md:rounded-[36px]">
          ...
          <header className="px-4 py-4 md:px-8 md:py-6 flex items-center justify-between border-b border-[rgba(23,32,51,0.08)] sticky top-0 bg-[rgba(255,255,255,0.72)] backdrop-blur-xl z-10 md:rounded-t-[36px]">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-mono-ui text-[10px] uppercase tracking-[0.28em] text-slate-500">
                  Biometric Pro
                </p>
          ...
          <div className="flex-1 px-4 py-6 md:px-8 md:py-8 space-y-6 md:space-y-8">
            ...
            {avals.length === 0 ? (
              <div className="glass-panel rounded-[24px] p-10 text-center text-slate-400">
                <p className="font-mono-ui text-sm mb-3 text-[color:var(--accent)]">
                  EMPTY
                </p>
                <p className="font-medium">Nenhuma avaliação ainda</p>
                <p className="text-sm mt-1">
                  Seu profissional realizará a primeira avaliação em breve.
                </p>
              </div>
            ) : (
              <>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {[
                    ...
                    return (
                      <div
                        key={c.label}
                        className={`metric-card rounded-[20px] bg-gradient-to-br ${c.tone} p-4 md:p-5 text-white`}
                      >
                ...
                {(r?.soma6Dobras != null || r?.somaTodasDobras != null) && (
                  <div className="grid grid-cols-2 gap-3">
                    <div className="metric-card rounded-[20px] bg-gradient-to-br from-[#ec4899] to-[#2563eb] p-4 md:p-5 text-white">
                      ...
                      {penultima?.resultado?.soma6Dobras != null &&
                        r?.soma6Dobras != null && (
                      ...
                      {r?.classificacao6Dobras && (
                        <span
                          className={`inline-block mt-2 px-2 py-0.5 rounded-lg text-xs font-bold font-mono-ui uppercase tracking-[0.12em] ${corDobras(r.classificacao6Dobras)}`}
                        >
                          {r.classificacao6Dobras}
                        </span>
                      )}
                    ...
                    <div className="metric-card rounded-[20px] bg-gradient-to-br from-[#2563eb] to-[#ec4899] p-4 md:p-5 text-white">
                      ...
                      {penultima?.resultado?.somaTodasDobras != null &&
                        r?.somaTodasDobras != null && (
                          <p className="text-xs mt-1 font-semibold text-white/80">
                            {r.somaTodasDobras >
                            penultima.resultado.somaTodasDobras
                              ? "▲"
                              : "▼"}{" "}
                            {Math.abs(
                              r.somaTodasDobras -
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {pontosSomato.length > 0 && (
                    <div className="glass-panel rounded-[24px] p-4 md:p-6">
                      <p className="font-mono-ui text-[10px] uppercase tracking-[0.22em] text-slate-400 mb-1">
                        Somatotipo
                      </p>
                      {r?.biotipo && (
                ...
                <div className="pb-6">
                  <p className="font-mono-ui text-[10px] uppercase tracking-[0.22em] text-slate-400 mb-3">
                    ...
                    <span className="ml-2 normal-case tracking-normal text-[10px] text-slate-300">
                    ...
                    {[...avals].reverse().map((a, i) => {
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <span
                              className={`w-2 h-2 rounded-full flex-shrink-0 ${i === 0 ? "bg-[color:var(--accent)]" : "bg-slate-200"}`}
                            />
                            <div>
